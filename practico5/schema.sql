...
ALTER TABLE customer ADD CHECK (customer.premium_customer = "T" OR customer.premium_customer = "F");
...
SELECT f.rating, COUNT(f.film_id) AS `film_count` FROM film f GROUP BY f.rating ORDER BY `film_count` DESC;
-- ¿Cuáles fueron la primera y última fecha donde hubo pagos?
SELECT MIN(p.payment_date) AS "Fecha primer pago", MAX(p.payment_date) AS "Fecha ultimo pago" FROM payment p;
-- Calcule, por cada mes, el promedio de pagos (Hint: vea la manera de extraer el nombre del mes de una fecha).
SELECT MONTHNAME(p.payment_date) AS `Mes`, AVG(p.amount) AS `Promedio de pagos` FROM payment p GROUP BY `Mes`;
